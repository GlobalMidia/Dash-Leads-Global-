...
import { Activity, AlertTriangle, Building2, Check, ClipboardList, HeartPulse, LayoutDashboard, LoaderCircle, Plus, ShieldCheck, X } from "lucide-react";
...
function isCurrentWeek(value: string | null) {
  return Boolean(value && value.slice(0, 10) >= reviewWeek());
}
...
export function HealthCenter({ initialAccounts, mode, user }: Props) {
  ...
  const reviewNeeded = accounts.filter((account) => !isCurrentWeek(account.lastReviewAt)).length;
  const fridayReminder = new Date().getDay() === 5 && reviewNeeded > 0;
  ...
  return <div className="dashboard-shell health-shell" data-contrast={preferences.highContrast ? "high" : "standard"} data-motion={preferences.reducedMotion ? "reduced" : "full"} data-text-size={preferences.textSize} data-theme={resolvedTheme}>
    ...
    {fridayReminder && <section className="health-weekly-reminder"><ClipboardList size={19}/><div><strong>Revisão semanal pendente</strong><small>{reviewNeeded} conta{reviewNeeded > 1 ? "s" : ""} ainda precisa{reviewNeeded === 1 ? "" : "m"} de atualização nesta sexta-feira.</small></div></section>}
