...
import { Activity, AlertTriangle, ArrowRight, BarChart3, Bell, Building2, Check, ClipboardList, ExternalLink, HeartPulse, History, LayoutDashboard, LoaderCircle, Pencil, Plus, RotateCcw, Search, Settings2, ShieldCheck, SlidersHorizontal, Users, X } from "lucide-react";
...
export function HealthCenter({ initialAccounts, initialNow, mode, user }: Props) {
  ...
  return <div className="dashboard-shell health-shell" data-contrast={preferences.highContrast ? "high" : "standard"} data-motion={preferences.reducedMotion ? "reduced" : "full"} data-text-size={preferences.textSize} data-theme={resolvedTheme}>
    ...
    <aside className="side-rail" aria-label="Navegação principal"><button className="rail-button" onClick={() => go("/")} title="Painel de leads" type="button"><LayoutDashboard size={19} /></button><button className="rail-button" onClick={() => go("/pme")} title="PME e reativação" type="button"><Building2 size={19} /></button><button className="rail-button active" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} title="Saúde das contas" type="button"><HeartPulse size={19} /></button><button className="rail-button" onClick={() => go("/meta-ads")} title="Meta Ads" type="button"><BarChart3 size={19} /></button><button className="rail-button" onClick={() => go("/google-ads")} title="Google Ads" type="button"><BarChart3 size={19} /></button><button className="rail-button" onClick={() => go("/ga4")} title="Google Analytics 4" type="button"><BarChart3 size={19} /></button><button className="rail-button" onClick={() => go("/#leads-table")} title="Leads" type="button"><Users size={19} /></button><button className="rail-button" onClick={() => go("/#filters")} title="Filtros" type="button"><SlidersHorizontal size={19} /></button><div className="rail-spacer" /><button className="rail-button" onClick={() => go("/")} title="Preferências" type="button"><Settings2 size={19} /></button><button className="rail-avatar" onClick={() => go("/")} title="Perfil" type="button">{user.initials}</button></aside>
